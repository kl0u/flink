...
public class AbstractUdfStreamOperatorLifecycleTest {
	...
	private static final List<String> EXPECTED_CALL_ORDER_FAILED_RUNNING = Arrays.asList(
			"OPERATOR::setup",
			"UDF::setRuntimeContext",
			"OPERATOR::initializeState",
			"OPERATOR::open",
	...
	private static final String ALL_METHODS_RICH_FUNCTION =
			"[" +
					...
					"getRuntimeContext[], " +
					"open[class org.apache.flink.configuration.Configuration], " +
					"setRuntimeContext[interface org.apache.flink.api.common.functions.RuntimeContext]" +
			"]";
	...
	@Test
	public void testLifeCycleException() throws Exception {
		...
		LifecycleTrackingFailingStreamSource.runStarted.await();

		// wait for clean termination
		task.getExecutingThread().join();
		assertEquals(ExecutionState.FAILED, task.getExecutionState());
		assertEquals(EXPECTED_CALL_ORDER_FAILED_RUNNING, ACTUAL_ORDER_TRACKING);
	}
	...
	private static class LifecycleTrackingFailingStreamSource<OUT, SRC extends SourceFunction<OUT>>
	...
	private static class MockExceptionThrowingFunction extends MockSourceFunction {
	...
	private static class MockSourceFunction extends RichSourceFunction<Long> {
		...
		@Override
		public void dispose() {
			ACTUAL_ORDER_TRACKING.add("UDF::dispose");
			// we are not calling the super.dispose() as
			// the default implementation calls the close().
		}
	}
	...
	private static class LifecycleTrackingStreamSource<OUT, SRC extends SourceFunction<OUT>>
		...
		LifecycleTrackingStreamSource(SRC sourceFunction, boolean simulateCheckpointing) {
			super(sourceFunction);
			this.simulateCheckpointing = simulateCheckpointing;
			runStarted = new OneShotLatch();
			runFinish = new OneShotLatch();
