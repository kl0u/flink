 ...
 */
@Internal
public class FlinkYarnSessionCliWithError extends AbstractCustomCommandLine {

	private static final String ID = "yarn-cluster";

	private final Option applicationId;

	public FlinkYarnSessionCliWithError(Configuration configuration) {
		super(configuration);
	...
	public boolean isActive(CommandLine commandLine) throws ClusterDeploymentException {
		if (originalIsActive(commandLine)) {
			throw new ClusterDeploymentException("A yarn-cluster can only be used with exported HADOOP_CLASSPATH." +
