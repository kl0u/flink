...
public class ExternalSort2ITCase extends TestLogger {
	
	private static final Logger LOG = LoggerFactory.getLogger(ExternalSort2ITCase.class);
	...
	public void testInMemorySort() {
		try {
			...
			Sorter<Tuple2<Integer, String>> merger = ExternalSorter.newBuilder(
				this.memoryManager,
				this.parentTask,
				this.pactRecordSerializer.getSerializer(),
				this.pactRecordComparator)
				.enableSpilling(this.ioManager)
				.memoryFraction((double)64/78)
				.startSpillingFraction(0.9f)
				.maxNumFileHandles(2)
				.largeRecords(true)
				.objectReuse(true)
				.build(source);
