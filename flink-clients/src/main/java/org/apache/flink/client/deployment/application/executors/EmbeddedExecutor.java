...
public class EmbeddedExecutor implements PipelineExecutor {
	...
	public CompletableFuture<JobClient> execute(final Pipeline pipeline, final Configuration configuration) {
		...
		if (optJobID.isPresent() && applicationJobIds.contains(optJobID.get())) {
			return getJobClientCompletableFuture(configuration, optJobID.get());
		}
		return submitAndGetJobClientCompletableFuture(pipeline, configuration);
	}

	private CompletableFuture<JobClient> getJobClientCompletableFuture(final Configuration configuration, final JobID jobID) {
	...
	private CompletableFuture<JobClient> submitAndGetJobClientCompletableFuture(final Pipeline pipeline, final Configuration configuration) {
