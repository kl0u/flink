...
public class ApplicationDispatcherBootstrapTest {
	...
	public void testClusterShutdownWhenApplicationIsCancelled() throws Exception {
		...
		bootstrap.stop(testingDispatcherGateway);
	...
	private List<JobID> submitAndGetJobIDs(int noOfJobs) throws FlinkException, InterruptedException {
		...
		final ApplicationDispatcherBootstrap bootstrap = new ApplicationDispatcherBootstrap(program, Collections.emptyList(), configuration);
		return bootstrap.getApplicationJobIDs(testingDispatcherGateway);
	}
	...
	private CompletableFuture<Void> getApplicationTerminationFuture(Map<JobID, CompletableFuture<JobResult>> jobResults) throws FlinkException {
